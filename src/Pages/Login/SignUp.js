...
const SignUp = () => {
    ...
    if(error || gError || updeteError){
      signInError= <p className='text-red-500'>{error?.message || gError?.message || updeteError?.message}</p>
    }
...
<input className='btn w-full max-w-xs text-white' type="submit" value="Sign Up" />
 ...
 class="btn btn-outline btn-info">Contineu with google</button>
 </div>
</div>
    </div>
    );
